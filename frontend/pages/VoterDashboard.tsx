  ...
  return (
    <div className="font-poppins text-slate-800 bg-slate-50 min-h-screen pb-8">
                ...
                {pos.candidates.map((candidate: any) => {
                  ...
                  return (
                    <div
                      ...
                      className={`group relative cursor-pointer w-full max-w-[280px] sm:max-w-none sm:w-[260px] lg:w-[280px] aspect-[3/4] rounded-[2rem] overflow-hidden transition-all duration-500 ease-out border-2
                        ${isSelected
                    ...
                    >
                      ...
                      <div className={`absolute top-4 right-4 w-8 h-8 rounded-full flex items-center justify-center z-20 transition-all duration-500 ${
                        isSelected ? 'bg-[#2f318d] text-white scale-100 opacity-100 shadow-md' : 'bg-white/90 backdrop-blur-sm text-slate-300 scale-75 opacity-0 group-hover:opacity-100'
                      }`}>
                        <Check size={16} strokeWidth={3} />
                      </div>

                      <div className="absolute bottom-0 left-0 right-0 z-10 p-3 pt-16 bg-gradient-to-t from-slate-900/50 via-slate-900/20 to-transparent">
                        <div className={`w-full rounded-[1.25rem] p-4 transition-all duration-500 flex flex-col items-start text-left ${
                          isSelected
                          ...
                          <span className={`inline-block px-2.5 py-1 rounded-md text-[0.55rem] md:text-[0.6rem] font-black tracking-widest uppercase transition-colors ${
                            isSelected ? 'bg-white/20 text-indigo-50' : 'bg-slate-100 text-slate-500 group-hover:bg-[#2f318d]/10 group-hover:text-[#2f318d]'
                          }`}>
                            {partylistName}
                          </span>
                        </div>
      ...
      <div className="fixed bottom-4 md:bottom-6 left-1/2 -translate-x-1/2 w-[95%] sm:w-[92%] max-w-2xl z-40 transition-all duration-500 animate-[slide-up_0.5s_ease-out]">
        <div className="bg-white/90 backdrop-blur-xl border border-slate-200/50 shadow-[0_20px_50px_rgba(47,49,141,0.15)] rounded-[1.5rem] md:rounded-[2rem] p-3 md:p-4 flex justify-between items-center px-5 md:px-8">
            ...
            <button
              ...
              className={`h-10 md:h-14 px-5 md:px-8 rounded-[1rem] md:rounded-[1.25rem] text-xs md:text-sm font-bold transition-all flex items-center gap-2 md:gap-2.5 active:scale-[0.98] ${
      ...
      {showReview && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/40 backdrop-blur-sm p-4 transition-opacity">
          <div className="w-full max-w-2xl bg-white rounded-[1.5rem] md:rounded-[2.5rem] shadow-2xl relative max-h-[90vh] md:max-h-[95vh] flex flex-col overflow-hidden animate-[scale-in_0.3s_ease-out]">
            ...
            <div className="p-5 md:p-8 overflow-y-auto flex-1 bg-slate-50/50">
                ...
                <div className="space-y-2.5 md:space-y-4">
                    {positions.map((pos) => {
                        ...
                        return (
                            <div key={pos.positionId} className={`flex items-center justify-between p-3 md:p-5 rounded-xl md:rounded-2xl border transition-colors ${
                              candidate ? 'bg-white border-indigo-50 shadow-sm' : 'bg-slate-50 border-slate-100 border-dashed'
                            }`}>
